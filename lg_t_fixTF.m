...
h = 0.05; % numerical diffentiation parameter
...
[t, y_predl] = ode23s(@(t,y)Pa_nss4(t,y,pars,TF-TF*h),tspan,y0);
[t, y_predr] = ode23s(@(t,y)Pa_nss4(t,y,pars,TF+TF*h),tspan,y0);
...
% calculate log gain at each time point
log_gain = zeros(length(tspan),3);
for j = 1:length(tspan)
    lg = gradient(log([y_predl(j) y_pred(j) y_predr(j)]))./gradient(log([TF-TF*h TF TF+TF*h]));
    log_gain(j,:) = lg;
end
